...
employed to prove the security of the One-Time Pad encryption scheme against a particular kind of attack.  This material
is standard~\cite{introtocrypto}; however, since we will perform the rest of the development in a functional programming
language, we have chosen to formulate this introduction in Haskell as well, rather than in an (imperative) pseudocode.
Fortunately, the resulting code is very similar thanks to the use of monads.

\subsection{Encryption Schemes}

An \emph{encryption scheme} is a tuple $\sigma = (K, M_p, M_c, e, d)$ where $K$ and $M_p$ are finite sets, $e : K \times
M_p \to M_c$, $d : K \times M_c \to M_p$ and for any $k \in K$ and $m \in M_p$, $d(k, e(k, m)) = m$.  We can regard $K$
as the set of keys, $M_p$ and $M_c$ as the sets of plaintext and ciphertext messages, and $e$ and $d$ as the encryption
and decryption functions.  We allow $e$ and $d$ to be non-deterministic functions.

We say that an encryption scheme $\sigma$ is \emph{secure} iff for any (non-deterministic) function $f : M_c \to M_p$,
the probability that $f(e(k, m)) = m$ is $\abs{M_p}^{-1}$, with $k$ sampled uniformly from $K$ and $m$ sampled uniformly
from $M_p$.  In other words, any function $f$ that does not depend on the choice of key does no better a job of
...
and error-prone~\cite{codebasedgames}.  However, note that if $m_0, m_1 \in M_p$ with $m_0 \neq m_1$, then for every $f
: M_c \to M_p$, the probability that $f(e(k, m)) = m$ is $\frac 1 2$, with $k$ chosen uniformly from $K$ and $m$ chosen
uniformly from $\{m_0, m_1\}$.  If this were not so, then $f$ could return a uniformly random $m \in M_p$ for inputs
other than $e(k, m_0)$ and $e(k, m_1)$ and choose better than random for inputs of that form.  On the other hand, if the
adversary cannot distinguish between the ciphertexts of any two particular messages, then it certainly cannot decode any
ciphertext.
...
$f(m_0, m_1, e(k, m_b)) = b$ is $\frac 1 2$, with $(m_0, m_1)$ sampled from $D$, $k$ sampled uniformly from $K$, and $b$
...
fixed set of computations which define the game.  The adversary must conform to the protocol, but can otherwise perform
any computations.
...
order to obtain the two messages and to get $b'$, then return $\neg b'$ in step 5.  Since |adv'| wins whenever |adv|
loses, the probability $p'$ is $1-p > 0.5$, as desired.  The existence of a worse-than-random adversary is thus as much
evidence of the scheme being insecure as the existence of a better-than-random adversary.
...
Computations performed by the encryption scheme and adversary are given as parameters to the program.  This is expressed
...
The above defines a syntax for games.  If we have a monad |Rnd| with support for randomness, then this syntax has a
natural valuation in the |StateT as Rnd| monad.  We say that two games are indistinguishable if they are equivalent
under this valuation; that is, if sampling some value from one has the same probability as sampling that value from the
other.  

\section{Example: One-Time Pad}
\label{sec:example}

Let us now put the above together to show that the One-Time XOR Pad encryption scheme is secure against eavesdropping,
but not secure if the adversary has access to the encryption function.
...
using a |BitVector| type family indexed by the length of the string.}
...
since the latter is a bijective mapping, so |fmap (\ k -> (xor k m0)) (uniform n)| is the same distribution as
...
thus as a whole indistinguishable from |flipCoin|.  We thus have a sequence of indistinguishable games starting with
|eav_game| and ending with |flipCoin|, showing that those two are indistinguishable, which is what we set out to prove.
...
The algorithm itself is straightforward, but an interesting question is why the proof we used for the Eavesdropper case
breaks down.  The problem with that proof is that |k| is now used not only for the generation of |m'|, but is also
passed (indirectly) to |chooseOutcome|.  A certain relation holds between |m'| and |k|, and thus if we replace |encrypt
k (if b then m0 else m1)| with |uniform n| then we must also replace all later appearances of |k| with |(xor m' (if b
then m0 else m1))| to maintain this relationship.  Doing this, however, brings the proof no further, since the
|chooseOutcome| call still depends on |b|, and an essential step of the proof was making the adversary commit to a |b'|
before |b| was chosen.
