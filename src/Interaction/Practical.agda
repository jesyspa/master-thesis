...
CommandF  (fmap-Augment* IS₁ IS₂ ps m) (left  c) = left (CommandF m c)
CommandF  (fmap-Augment* IS₁ IS₂ ps m) (right c) = right c
ResponseF (fmap-Augment* IS₁ IS₂ ps m) {left  c} r = ResponseF m r
ResponseF (fmap-Augment* IS₁ IS₂ ps m) {right c} r = r

embed-Augment** : (IS : InteractionStructure)(pss : List PlayerSig) → ISMorphism IS (Augment** IS pss)
embed-Augment** IS [] = id-IS
embed-Augment** IS (ps ∷ pss) = comp-IS (embed-Augment* IS ps) (fmap-Augment* IS (Augment** IS pss) ps (embed-Augment** IS pss))
...
module _ (IS₁ IS₂ : InteractionStructure) where
  ...
  resolve-impl {(arg , ret) ∷ ps} (right c) (Method m impl) = resolve-impl c impl

  resolve* : ∀{A}{ps : PlayerSig} → FreeMonad (Augment* IS₁ ps) A → Impl IS₂ ps → FreeMonad (Product-IS IS₁ IS₂) A
  resolve* (Return-FM a) impl = Return-FM a
  resolve* (Invoke-FM (left  c) cont) impl = Invoke-FM (left c) λ r → resolve* (cont r) impl
  resolve* (Invoke-FM (right c) cont) impl = bind-IS (action-IS (Incl-R IS₁ IS₂) (resolve-impl c impl)) λ r → resolve* (cont r) impl

  resolve*1 : ∀{A}{ps : PlayerSig}{pss : List PlayerSig}
            → FreeMonad (Augment** IS₁ (ps ∷ pss)) A → Impl IS₂ ps
            → FreeMonad (Augment** (Product-IS IS₁ IS₂) pss) A
  resolve*1 (Return-FM a) impl = Return-FM a
  resolve*1 {ps = ps}{pss} (Invoke-FM (left  c) cont) impl
    = Invoke-FM (embed-Interchange IS₁ IS₂ pss c)
                λ r → resolve*1 {ps = ps} {pss} (cont (undo-embed-Interchange IS₁ IS₂ pss c r)) impl
  resolve*1 {ps = ps}{pss} (Invoke-FM (right c) cont) impl
    = bind-IS (action-IS (comp-IS (Incl-R IS₁ IS₂) (embed-Augment** (Product-IS IS₁ IS₂) pss)) (resolve-impl c impl))
              λ r → resolve*1 {ps = ps}{pss} (cont r) impl 
