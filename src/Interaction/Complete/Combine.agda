...
module _ {def₁ def₂ : PlayerDef} where
  ...
  Combine-FM plr (Invoke-FM (true  , (name , arg)) cont) = bind-FM (fmap-FM (InclR-IS _ _) (plr name arg)) λ r → Combine-FM plr (cont r)

  Combine : Impl-PD (Extend-PD def₁ def₂)
          → Impl-PD def₂
          → Impl-PD (Join-PD def₁ def₂)
  Combine plr₁ plr₂ (false , name) = fmap-MI (Combine-FM plr₂) (plr₁ name)
  Combine plr₁ plr₂ (true  , name) = (fmap-MI ∘′ fmap-FM) (InclR-IS _ _) (plr₂ name)
