...
module _ {l S}(IS : IStruct S){𝑺 : Set l} where
  record Implementation (M : (𝑺 → Set l) → 𝑺 → Set l)(StateF : S → 𝑺) : Set l where
    field
...
module _ {l S}{IS : IStruct S}{𝑺 : Set l}{M : (𝑺 → Set l) → 𝑺 → Set l}{{_ : IxMonad M}}{StateF : S → 𝑺} where
  ...
  TermM (uprop-Impl impl) (Invoke-FM c cont) = RunImpl impl c >>=ⁱ λ { (StrongV r refl) → TermM (uprop-Impl impl) (cont r) }
...
module _ {S₁ S₂ S₃}{IS₁ : IStruct S₁}{IS₂ : IStruct S₂}{IS₃ : IStruct S₃}{f g} where
  ...
  RunImpl (comp-SI si sj) x = TermM (fmap-SynImpl-FM sj) (fmapⁱ (λ { (StrongV r refl) → StrongV r refl }) (RunImpl si x))
...
module _ {S₁ S₂ T₁ T₂}
  ...
  RunImpl (binmap-SI si sj) {s , t} (left  c)
    = TermM (fmap-IS-FM $ IncL-IS (StateF₂ t)) $ fmapⁱ (λ { (StrongV x refl) → StrongV x refl }) $ RunImpl si c
  RunImpl (binmap-SI si sj) {s , t} (right c)
    = TermM (fmap-IS-FM $ IncR-IS (StateF₁ s)) $ fmapⁱ (λ { (StrongV x refl) → StrongV x refl }) $ RunImpl sj c
