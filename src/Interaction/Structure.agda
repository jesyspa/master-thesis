...
module _ where
  ...
  record ISMorphism (IS₁ IS₂ : InteractionStructure)  : Set₁ where
    field
      ...
      ResponseF : ∀{s c} → Response IS₂ (StateF s) (CommandF c) → Response IS₁ s c
      nextF     : ∀ s c r → StateF (next IS₁ s c (ResponseF r)) ≡ next IS₂ (StateF s) (CommandF c) r
  ...
  nextF     (comp-IS m₁ m₂) s c r
    rewrite nextF m₁ s c (ResponseF m₂ r) | nextF m₂ (StateF m₁ s) (CommandF m₁ c) r
                               = refl
...
module _ where
  ...
  ISMorphism-action {IS₁} {IS₂} {A} {s} m (Invoke-FIXM c cont) = Invoke-FIXM (CommandF m c) λ r → {!!}
    where
      lem : (r : Response IS₂ (StateF m s) (CommandF m c)) → FreeIxMonad IS₂ A (next IS₂ (StateF m s) (CommandF m c) r)
      lem r rewrite sym (nextF m s c r) = ISMorphism-action m (cont (ResponseF m r))
