...
fmap-ext-CE f g pf (addOracle-CE ce cf) = cong (addOracle-CE ce) $ fmap-ext-CE f g pf cf
...
fmap-id-CE (addOracle-CE ce cf) = cong (addOracle-CE ce) $ fmap-id-CE cf
...
fmap-comp-CE g f (addOracle-CE ce cf) = cong (addOracle-CE ce) $ fmap-comp-CE g f cf
...
fmap-embed-CE f (addOracle-CE ce cf) = cong (addOracle-CE ce) $ fmap-embed-CE f cf
...
cofmap-ext-CE f g pf (addOracle-CE ce cf) = cong (λ e → addOracle-CE e cf) $ cofmap-ext-CE f g pf ce
...
cofmap-id-CE (addOracle-CE ce cf) = cong (λ e → addOracle-CE e cf) $ cofmap-id-CE ce
...
cofmap-comp-CE g f (addOracle-CE ce cf) = cong (λ e → addOracle-CE e cf) $ cofmap-comp-CE g f ce 
...
cofmap->>>-dist f (addOracle-CE ce cf) F = refl
...
fmap-cofmap-swap f g (addOracle-CE ce cf) = refl
...
id->>>-right-unit (addOracle-CE ce cf) = cong (addOracle-CE ce) $ id->>>-right-unit cf
...
id->>>-left-unit (addOracle-CE E E′) = cong (λ e → addOracle-CE e E′) $ id->>>-left-unit E
...
>>>-assoc (addOracle-CE E E′) F G = cong (addOracle-CE E) $ >>>-assoc E′ F G
...
embed-interchangeable f (uniform-CE n ce) = cong (uniform-CE n) $
  ...
  ∎
-- I have the suspicion these two cases are not actually provable due to different choices of C.  Bummer.
embed-interchangeable f (addOracle-CE ce cf) =
  {!!}
    ≡⟨ {!!} ⟩
  {!!}
  ∎
