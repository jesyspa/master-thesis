...
module Syntactic.BoundedOracleUse (ST : Set)(OD : OracleData) where
...
module _ {{_ : Enumeration ST}}
  ...
  check-BOU? : ∀{A} → Bool → Nat → (CryptoOracleExpr ST A) → Bool
  ...
  check-BOU-Sound : ∀{A} b k → (ce : CryptoOracleExpr ST A) → IsTrue (check-BOU? b k ce) → BoundedOracleUse b k ce
  ...
  check-BOU-Sound b (suc k) (Invoke-FM (true , CallOracle arg) cont) pf
    = CallOracleBOU b k arg cont λ r → check-BOU-Sound b k (cont r) {!!}
