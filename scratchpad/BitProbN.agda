...
data Prob (n : Nat) : Set where
  never always : Prob n
  sometimes : ProbPath → Prob n

weaken-prob : ∀{n} → Prob n → Prob (suc n)
weaken-prob p = {!!} 

weaken-prob-k : ∀{n} → (k : Nat) → Prob n → Prob (k +N n)
weaken-prob-k k p = {!!}
...
data _≤P_ {n : Nat} : Prob n → Prob n → Set where
...
P-neg : ∀{n} → Prob n → Prob n
P-neg never = always
P-neg always = never
P-neg (sometimes p) = sometimes (PP-neg p)

P-avg : ∀{n} → Prob n → Prob n → Prob n
...
_*P_ : ∀{n m} → Prob n → Prob m → Prob (n +N m)
never *P q = never
_*P_ {n} always q = weaken-prob-k n q
p *P never = never
_*P_ {n} {m} p always rewrite nat-commute-lem n m = weaken-prob-k m p
sometimes p *P sometimes q = sometimes (PP-mul p q)

infixl 7 _*NP_
-- Multiplication with a natural number
_*NP_ : ∀{n : Nat} → (k : Nat) → Prob n → Prob {!!} 
zero *NP p = never
suc k *NP p = {!!} 
